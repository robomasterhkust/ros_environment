...
, m_sync_offset_for_debug( 0.0 )
{
    ...
    for ( int i = 0; i < cam_cnt; i++ )
    {
        ...
        pnode.param( prefix + "aec_desired_gray_value", cs.aec_desired_gray_value, aec_desired_gray_value );
    ...
    "sync_offset", 10, boost::bind( &Camera::debug_sync_offset_callback, this, _1 ),
...
bool
Camera::is_slave_mode( ) const
{
    return m_is_slave_mode;
}

bool
Camera::is_fast_mode( ) const
{
    return m_fast_mode;
}
...
bool
Camera::initSingleMVDevice( unsigned int id, const CameraSetting& cs )
{
    ...
    {
        std::cout
        << "An error occurred while initializing the statistical information on device "
        << devMgr[id]->serial.read( ) << "(error code: " << e.getErrorCode( ) << "("
        << e.getErrorCodeAsString( ) << "))." << std::endl
        << "Press [ENTER] to end the application..." << std::endl;
        return false;
    ...
    // Auto exposure, modified controller for better results, be careful about the minimum
    // exposure
    // time
    if ( cs.use_auto_exposure )
    {
        ...
        settings.cameraSetting.autoControlParameters.desiredAverageGreyValue.write( cs.aec_desired_gray_value );
        settings.cameraSetting.autoControlParameters.controllerDelay_Images.write( cs.aec_control_delay_frame );
        ...
        ROS_INFO(
        "Auto Exposure w/ Max Exposure Time (us) :  %d . Desired Grey Value: %d",
    ...
    // prefill the capture queue. There can be more then 1 queue for some device, but only
    // one for
    // now
...
bool
Camera::wait_for_imu_ack( SyncAckInfo& sync_ack, int& queue_size )
{
...
Camera::process_fast_sync( )
{
    ...
    while ( pnode.ok( ) )
    {
        ...
        ROS_INFO_COND( m_verbose_output, "Try grabing data with imu_seq[%d] grab_seq[%d]",
        ...
        if ( grab_result )
        {
            for ( auto& item : img_publisher )
            {
                ...
                // After artificial calibrated, we found that actually image is taken one
                // imu later
                // than triggered
...
void
Camera::debug_sync_offset_callback( const std_msgs::Float64ConstPtr& pMsg )
{
