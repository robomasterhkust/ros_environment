...
public:
	using ReceivedCb = std::function<void (const mavlink_message_t *message, const Framing framing)>;
...
protected:
	...
	static std::unordered_map<msgid_t, const mavlink_msg_entry_t*> message_entries;

	//! Channel number used for logging.
	size_t conn_id;

	inline mavlink_status_t *get_status_p() {
		return &m_status;
	}

	inline mavlink_message_t *get_buffer_p() {
		return &m_buffer;
	}
